...
public enum UMaterial {
	/*
	 * <item>(1.8.9, 1.9.4, 1.10.2, 1.11.2, 1.12.2, 1.13.2, 1.14.3) 1.8.9 =
	...
	SPLASH_POTION_FIRE_RESISTANCE_EXTENDED(PotionBase.SPLASH, "FIRE_RESISTANCE", true, false, "POTION",
			"SPLASH_POTION"),
	...
	SPLASH_POTION_WATER_BREATHING_EXTENDED(PotionBase.SPLASH, "WATER_BREATHING", true, false, "POTION",
			"SPLASH_POTION"),
	...
	private static final HashMap<String, UMaterial> inUMemory = new HashMap<String, UMaterial>();
	private static final HashMap<String, ItemStack> inMemory = new HashMap<String, ItemStack>();
	...
	public static ItemStack getEnchantmentBook(Enchantment enchant, int level, int amount) {
		final LinkedHashMap<Enchantment, Integer> e = new LinkedHashMap<Enchantment, Integer>();
		e.put(enchant, level);
	...
	public static UMaterial match(ItemStack item) {
		if (item != null && !item.getType().equals(Material.AIR)) {
			...
			if (version.contains("1.8") || version.contains("1.9") || version.contains("1.10")
				...
				final UMaterial u = match(un, d);
				un = u != null ? u.name() : null;
				if (un != null)
					return valueOf(un);
			}
			return un != null ? un.contains("SPAWN_EGG") ? matchSpawnEgg(item)
					: un.contains("ENCHANTED_BOOK") ? matchEnchantedBook(item) : valueOf(un) : null;
		}
		return null;
	}
